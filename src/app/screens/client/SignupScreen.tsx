...
export function SignupScreen() {
  ...
  // Cosmetic only - there's no billing integration on the salon side yet.
  // Salon owner signup is subscription-framed for now, so this exists to
  // set that expectation, not to actually charge anyone.
  ...
  const handleVerifySubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (signupType === "salon") {
        // Already fully authenticated at this point (tokens are set above) -
        // plan/payment below are cosmetic only, so there's no second login
        // needed once they get through it, unlike the previous version of
        // this flow which tried to log in again there and referenced a
  ...
  const handlePaymentSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // Fake payment processing - no billing integration on the salon side
    // yet. The account is already fully signed in from the verify step, so
    // this just simulates a brief "processing" delay before landing them
    // on the dashboard, rather than actually charging the card fields above.
    setIsSubmitting(true);
    setTimeout(() => {
      setIsSubmitting(false);
      navigate("/salon");
  ...
  // Step 3: Subscription plan selection (salon only - cosmetic, no billing
  // integration yet)
  if (step === "plan") {
    return (
      <div className="h-screen flex flex-col bg-gradient-to-br from-gray-50 to-white dark:from-gray-900 dark:to-gray-800 max-w-md mx-auto overflow-y-auto">
        <div className="flex-1 px-8 py-12">
          ...
          <p className="text-center mt-6 text-xs text-gray-400 dark:text-gray-500 leading-relaxed">
            You're already signed in - this step is just to pick a plan before continuing to your dashboard.
          </p>
        </div>
      </div>
    );
  ...
  // Step 4: Payment information (salon only - fake, same reasoning as above)
  if (step === "payment") {
          ...
          <form onSubmit={handlePaymentSubmit} className="space-y-4">
            ...
            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-4 mt-6">
              <p className="text-xs text-blue-800 dark:text-blue-300 leading-relaxed">
                🔒 This is a demo signup flow - no real charge will be made and no card details are sent anywhere.
              </p>
            </div>
